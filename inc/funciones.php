                ...
                case 'salario':
                  {
                    ...
                    if (isset($_SESSION['permitidos']) AND in_array($var, $_SESSION['permitidos']) AND in_array("2".$var, $_SESSION['permisos'])) {
                      ...
                      //$salario    = $query->queryJson("SELECT COD_FUNC AS id, MES+'-'+CONVERT(VARCHAR(10), YEAR(PERIODO)) AS periodo, SALARIO_BRUTO_FIJO AS fijo, SALARIO_BRUTO_VARIABLE AS variable, TOTAL AS total, AGUINALDO AS aguinaldo, APORTE_PATRONAL AS aporte FROM COLABORADOR_SALARIO WHERE CONVERT(date, PERIODO, 23) > '$fechaInicio' AND CONVERT(date, PERIODO, 23) <= '$fechaFin' AND COD_FUNC = $var ORDER BY CONVERT(DATE, PERIODO) DESC", "salario");
