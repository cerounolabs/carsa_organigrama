...
CASE 
    ...
    END AS MES,
B_1.ANHO AS AÑO,
ISNULL(C_1.MONTO_COBRADO_SAL_FIJO_DETALLE, 0) AS SALARIO_BRUTO_FIJO,
(ISNULL(C_2.MONTO_COBRADO_SAL_VARIABLE_DETALLE, 0) + ISNULL(C_3.MONTO_COBRADO_SAL_MANUAL_DETALLE, 0)) AS SALARIO_BRUTO_VARIABLE,
(ISNULL(C_1.MONTO_COBRADO_SAL_FIJO_DETALLE, 0) + ISNULL(C_2.MONTO_COBRADO_SAL_VARIABLE_DETALLE, 0) + ISNULL(C_3.MONTO_COBRADO_SAL_MANUAL_DETALLE, 0)) AS TOTAL,
ROUND(((ISNULL(C_1.MONTO_COBRADO_SAL_FIJO_DETALLE, 0) + ISNULL(C_2.MONTO_COBRADO_SAL_VARIABLE_DETALLE, 0) + ISNULL(C_3.MONTO_COBRADO_SAL_MANUAL_DETALLE, 0)) / 12), 0) AS AGUINALDO,
CASE 
	WHEN D_1.REGIMEN_CODIGO = 2 THEN ROUND((((ISNULL(C_1.MONTO_COBRADO_SAL_FIJO_DETALLE, 0) + ISNULL(C_2.MONTO_COBRADO_SAL_VARIABLE_DETALLE, 0) + ISNULL(C_3.MONTO_COBRADO_SAL_MANUAL_DETALLE, 0)) * 25.5) / 100), 0) 
...
LEFT OUTER JOIN (
    SELECT YEAR(A.FuncFecHas) AS ANHO, MONTH(A.FuncFecHas) AS MES, A.FuCod, A.FuncFecHas AS FECHA_CORTE, B.RR01Id AS TIPO_LIQ_COD_SAL_FIJO, B.RR01Des AS TIPO_LIQ_DESC_SAL_FIJO, CAST(A.FuncCredTo AS numeric) AS MONTO_COBRADO_SAL_FIJO, (SELECT SUM(C.FuncMont) FROM dbo.CPTOFUNC1 AS C WHERE A.FuncCod = C.FuncCod AND A.FuCod = C.FuCod AND A.FunCIC = C.FunCIC AND A.FuncFecDes = C.FuncFecDes AND A.FuncFecHas = C.FuncFecHas AND C.CptoCod IN (1)) AS MONTO_COBRADO_SAL_FIJO_DETALLE
    FROM dbo.CPTOFUNC AS A
...
LEFT OUTER JOIN (
    SELECT YEAR(A.FuncFecHas) AS ANHO, MONTH(A.FuncFecHas) AS MES, A.FuCod, A.FuncFecHas AS FECHA_CORTE, B.RR01Id AS TIPO_LIQ_COD_SAL_VARIABLE, B.RR01Des AS TIPO_LIQ_DESC_SAL_VARIABLE, CAST(A.FuncCredTo AS numeric) AS MONTO_COBRADO_SAL_VARIABLE, (SELECT SUM(C.FuncMont) FROM dbo.CPTOFUNC1 AS C WHERE A.FuncCod = C.FuncCod AND A.FuCod = C.FuCod AND A.FunCIC = C.FunCIC AND A.FuncFecDes = C.FuncFecDes AND A.FuncFecHas = C.FuncFecHas AND C.CptoCod IN (1)) AS MONTO_COBRADO_SAL_VARIABLE_DETALLE
    FROM dbo.CPTOFUNC AS A
...
LEFT OUTER JOIN (
    SELECT YEAR(A.FuncFecHas) AS ANHO, MONTH(A.FuncFecHas) AS MES, A.FuCod, A.FuncFecHas AS FECHA_CORTE, B.RR01Id AS TIPO_LIQ_COD_SAL_MANUAL, B.RR01Des AS TIPO_LIQ_DESC_SAL_MANUAL, CAST(A.FuncCredTo AS numeric) AS MONTO_COBRADO_SAL_MANUAL, (SELECT SUM(C.FuncMont) FROM dbo.CPTOFUNC1 AS C WHERE A.FuncCod = C.FuncCod AND A.FuCod = C.FuCod AND A.FunCIC = C.FunCIC AND A.FuncFecDes = C.FuncFecDes AND A.FuncFecHas = C.FuncFecHas AND C.CptoCod IN (1)) AS MONTO_COBRADO_SAL_MANUAL_DETALLE
    FROM dbo.CPTOFUNC AS A
